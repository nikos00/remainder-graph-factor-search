...
Thin wrapper that selects experiment number(s) and calls
run_experiments_cli_streaming_rootsN.py with the corresponding
parameter/primes files.

Unlike run_par.py, this wrapper runs sequentially (single process).

Also ALWAYS enables --hard-timelimit when invoking the runner.
"""
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(
        description="Run experiment(s) by number via run_experiments_cli_streaming_rootsN.py (sequential over (s,p) pairs)."
    )
    parser.add_argument(
        "--exp",
        required=True,
